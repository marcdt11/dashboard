...
## This Repo
- **`marcdt11/dashboard`** → served at `dashboard.marctorrence.com` (GitHub Pages, `main` branch, root path).
- Deliberately separate from the `marcdt11/poker` repo (`poker.marctorrence.com`) so the dashboard is not discoverable from the public poker tools site.
- Single self-contained `index.html` (vanilla HTML/CSS/JS), no build step. Reads a published Google Sheet CSV client-side.
...
- **`PROJECT.md`** must stay in sync with the codebase. Update it **in the same commit** as the code change, not as a follow-up. Specifically update:
  ...
  - Architecture section when services or data flow change
